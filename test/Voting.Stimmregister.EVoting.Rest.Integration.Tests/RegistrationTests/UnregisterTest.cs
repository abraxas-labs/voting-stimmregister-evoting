...
public class UnregisterTest : BaseRestTest
{
    private const string UnregisterApiUrl = "v1/unregister";
    ...
    public UnregisterTest(TestApplicationFactory factory)
        : base(factory)
    {
    }
    ...
    public async Task ShouldWorkWithExistingActiveStatusChange()
    {
        ...
        var statusChanges = await RunOnDb(db =>
        {
            return db.EVotingStatusChanges
                ...
                .OrderByDescending(c => c.EVotingRegistered)
                .ToListAsync();
        });
        ...
        registered.EVotingRegistered.Should().BeFalse();
    ...
    [Fact]
    public async Task ShouldWorkWhenNotRegisteredWithPendingEmailVerification()
    {
        await RunOnDb(async db =>
        {
        ...
        using var resp = await Unregister(Ahvn13MockedData.Ahvn13Valid1Formatted, BfsCantonMockedData.BfsCantonEmailRequired, dateOfBirth);
        resp.StatusCode.Should().Be(HttpStatusCode.OK);

        var hasStatusChanges = await RunOnDb(db => db.EVotingStatusChanges.AnyAsync(x => x.Person!.Ahvn13 == Ahvn13MockedData.Ahvn13Valid1));
        hasStatusChanges.Should().BeFalse();

        var hasEmailVerifications = await RunOnDb(db => db.EmailVerifications.AnyAsync(x => x.Ahvn13 == Ahvn13MockedData.Ahvn13Valid1));
        hasEmailVerifications.Should().BeFalse();
    }
    ...
    [Fact]
    public async Task ShouldReturnErrorWhenNotRegisteredAndEmailVerificationExpired()
    {
        await RunOnDb(async db =>
        {
            db.EmailVerifications.Add(new EmailVerificationEntry
    ...
    public async Task ShouldReturnErrorWhenDateOfBirthDoesNotMatch()
    {
        ...
        using var resp = await Unregister(
    ...
    {
        await AssertStatus(
            () => Unregister(string.Empty, BfsCantonMockedData.BfsCantonValid, new DateOnly(1950, 01, 23)),
    ...
    {
        await AssertStatus(
            () => Unregister(Ahvn13MockedData.Ahvn13InvalidChecksumFormatted, BfsCantonMockedData.BfsCantonValid, new DateOnly(1950, 01, 23)),
            HttpStatusCode.BadRequest);
    }
    ...
    {
        await AssertStatus(
            () => AdminClient.PostAsJsonAsync(UnregisterApiUrl, new { }),
            HttpStatusCode.BadRequest);
    }
    ...
    private async Task<HttpResponseMessage> Unregister(string ahvn13, short bfsCanton, DateOnly dateOfBirth)
    {
